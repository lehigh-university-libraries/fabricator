...
func TestRunResolveUnpublishedSupplementalMergesExistingAddMediaRowsAsPublished(t *testing.T) {
	inputDir := filepath.Join(t.TempDir(), "input_data")
	if err := os.Mkdir(inputDir, 0755); err != nil {
		t.Fatalf("failed to create input dir: %v", err)
	}
	...
	files := map[string]string{
		"target.add_media.csv": "node_id,file\n3000,/mnt/islandora_staging/public.pdf\n",
		"target.unpublished_supplemental.csv": strings.Join([]string{
			"id,node_id,file,media_use_tid,published",
			",3000,/mnt/islandora_staging/private.pdf,151326,0",
			"",
		}, "\n"),
	}
	for name, content := range files {
	...
	expected := strings.Join([]string{
		"node_id,file,media_use_tid,published",
		"3000,/mnt/islandora_staging/public.pdf,,1",
		"3000,/mnt/islandora_staging/private.pdf,151326,0",
		"",
	}, "\n")
	if got != expected {
		t.Fatalf("unexpected merged add_media CSV:\n%s", got)
	}
}
